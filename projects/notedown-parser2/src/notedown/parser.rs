...
fn parse_markdown_inline(i: &str) -> IResult<&str, NotedownInline> {
    alt((
        map(parse_italics, |s: &str| {
            NotedownInline::Italic(s.to_string())
        }),
        map(parse_inline_code, |s: &str| {
            NotedownInline::InlineCode(s.to_string())
        }),
        map(parse_boldtext, |s: &str| {
            NotedownInline::Bold(s.to_string())
        }),
        map(parse_image, |(tag, url): (&str, &str)| {
            NotedownInline::Image(tag.to_string(), url.to_string())
        }),
        map(parse_link, |(tag, url): (&str, &str)| {
            NotedownInline::Link(tag.to_string(), url.to_string())
        }),
        map(parse_plaintext, |s| NotedownInline::Plaintext(s)),
    ))(i)
}
...
fn test_parse_markdown_inline() {
    assert_eq!(
        ...
        Ok(("", NotedownInline::Italic(String::from("here is italic"))))
    );
    assert_eq!(
        parse_markdown_inline("**here is bold**"),
        Ok(("", NotedownInline::Bold(String::from("here is bold"))))
    );
    assert_eq!(
        parse_markdown_inline("`here is code`"),
        Ok(("", NotedownInline::InlineCode(String::from("here is code"))))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Ok((
            "",
            (NotedownInline::Image(String::from("alt text"), String::from("image.jpg")))
        ))
    );
    assert_eq!(
        ...
        Ok((
            "",
            NotedownInline::Plaintext(String::from("here is plaintext!"))
        ))
    );
    assert_eq!(
        ...
        Ok((
            ...
            NotedownInline::Plaintext(String::from("here is some plaintext "))
        ))
    );
    assert_eq!(
...
fn test_parse_markdown_text() {
    ...
    assert_eq!(
        ...
        Ok((
            "",
            vec![NotedownInline::Plaintext(String::from(
    ...
    assert_eq!(
        ...
        Ok((
            ...
            vec![
                NotedownInline::Plaintext(String::from("here is some plaintext ")),
                NotedownInline::Italic(String::from("but what if we italicize?")),
            ]
        ))
    );
    assert_eq!(
        ...
        Ok(("", vec![
            NotedownInline::Plaintext(String::from("here is some plaintext ")),
            NotedownInline::Italic(String::from("but what if we italicize?")),
            NotedownInline::Plaintext(String::from(" I guess it doesnt ")),
            NotedownInline::Bold(String::from("matter")),
            NotedownInline::Plaintext(String::from(" in my ")),
            NotedownInline::InlineCode(String::from("code")),
        ]))
    );
    assert_eq!(
        parse_markdown_text("here is some plaintext *but what if we italicize?*\n"),
...
fn test_parse_header() {
    ...
    assert_eq!(
        ...
        Ok((
            "",
            (3, vec![NotedownInline::Plaintext(String::from(" h3"))])
        ))
    );
...
fn test_parse_unordered_list() {
    ...
    assert_eq!(
        ...
        Ok((
            "",
            vec![vec![NotedownInline::Plaintext(String::from(
                "this is an element"
            ))]]
        ))
    );
    assert_eq!(
        ...
        Ok((
            ...
            vec![
                ...
                vec![NotedownInline::Plaintext(String::from("here is another"))]
            ]
        ))
    );
}
...
fn test_parse_markdown() {
    assert_eq!(
        ...
        Ok(("", vec![
            Markdown::Heading(1, vec![NotedownInline::Plaintext(String::from("Foobar"))]),
            Markdown::Line(vec![]),
            Markdown::Line(vec![NotedownInline::Plaintext(String::from("Foobar is a Python library for dealing with word pluralization."))]),
            Markdown::Line(vec![]),
            Markdown::Codeblock(String::from("bash\n pip install foobar\n")),
            Markdown::Line(vec![]),
            Markdown::Heading(2, vec![NotedownInline::Plaintext(String::from("Installation"))]),
            Markdown::Line(vec![]),
            Markdown::Line(vec![
                NotedownInline::Plaintext(String::from("Use the package manager ")),
                NotedownInline::Link(String::from("pip"), String::from("https://pip.pypa.io/en/stable/")),
                NotedownInline::Plaintext(String::from(" to install foobar.")),
            ]),
